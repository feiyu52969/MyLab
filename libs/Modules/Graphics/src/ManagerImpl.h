...
//コード化されたシェーダオブジェクト
#include "src/NoLightingVsGenerated.h"
#include "src/VertexLightingVsGenerated.h"
#include "src/PixelLightingVsGenerated.h"
#include "src/PixelLightingPsGenerated.h"
...
public:
	...
	mMatricesChanged( true ){ //初回のdrawは行列も送ってくれ
		...
		//頂点シェーダ1.1行けてる？
#ifdef NEED_VS20
		bool vertexShaderReady = ( caps.VertexShaderVersion >= D3DVS_VERSION( 2, 0 ) );
#else
		bool vertexShaderReady = ( caps.VertexShaderVersion >= D3DVS_VERSION( 1, 1 ) );
#endif
		//ピクセル2.0行けてる？
		...
		createShader(
			...
			0,
			gShaderHlslNoLightingVsObj,
			sizeof( gShaderHlslNoLightingVsObj ) );
		createShader(
			&mVertexLightingVertexShader,
			0,
			gShaderHlslVertexLightingVsObj,
			sizeof( gShaderHlslVertexLightingVsObj ) );
		//ピクセルシェーダを使う場合
		if ( mPixelShaderReady ){
			createShader(
				&mPixelLightingVertexShader,
				0,
				gShaderHlslPixelLightingVsObj,
				sizeof( gShaderHlslPixelLightingVsObj ) );
			createShader(
				0,
				&mPixelLightingPixelShader,
				gShaderHlslPixelLightingPsObj,
				sizeof( gShaderHlslPixelLightingPsObj ) );
		}
	...
	int shaderObjSize ){
		...
		if ( outVs ){
			...
			STRONG_ASSERT( hr != D3DERR_INVALIDCALL && "CreateVertexShader : INVALID CALL" );
			STRONG_ASSERT( hr != D3DERR_OUTOFVIDEOMEMORY && "CreateVertexShader : OUTOF VIDEO MEMORY" );
			STRONG_ASSERT( hr != E_OUTOFMEMORY && "CreateVertexShader : E_OUTOFMEMORY" );
		}else{
			hr = mDevice->CreatePixelShader( shaderObj, outPs );
			STRONG_ASSERT( hr != D3DERR_INVALIDCALL && "CreatePixelShader : INVALID CALL" );
			STRONG_ASSERT( hr != D3DERR_OUTOFVIDEOMEMORY && "CreatePixelShader : OUTOF VIDEO MEMORY" );
			STRONG_ASSERT( hr != E_OUTOFMEMORY && "CreatePixelShader : E_OUTOFMEMORY" );
		}
	...
	void blendToScreen( Texture::Impl* t ){
		...
		Matrix34 wm;
		wm.setIdentity();
		float rw = static_cast< float >( t->width() ) / static_cast< float >( t->originalWidth() );
		float rh = static_cast< float >( t->height() ) / static_cast< float >( t->originalHeight() );
		wm.m00 = rw;
		wm.m11 = rh;
		wm.m03 = rw - 1.f;
		wm.m13 = 1.f - rh;
